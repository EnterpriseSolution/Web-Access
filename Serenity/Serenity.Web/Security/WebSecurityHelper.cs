...
namespace Serenity
{
    ...
    public static class WebSecurityHelper
    {
        ...
        public static bool Authenticate(ref string username, string password,string companyCode, bool persist)
        {
            if (username == null)
            ...
            var user = Dependency.Resolve<Serenity.Abstractions.IAuthenticationService>().Validate(ref username, password, companyCode);
            if (user==null)
                return false;
            
            SetAuthenticationTicket(username, companyCode, persist);
            HttpRuntime.Cache["COMPANY_INFO"] = user;
        ...
        public static void SetAuthenticationTicket(string username, string companyCode, bool persist)
        {
            if (username == null)
                throw new ArgumentNullException(username);
...
//#else
            ...
            //HttpContext.Current.Cache["COMPANY_CODE"] = companyCode;
