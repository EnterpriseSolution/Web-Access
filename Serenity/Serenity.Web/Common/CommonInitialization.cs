...
namespace Serenity.Web
{
    public static class CommonInitialization
    {
        ...
        public static void InitializeDynamicScripts()
        {
            ...
            new TemplateScriptRegistrar().Initialize(new[] { "~/Views/Templates", "~/Modules" }, watchForChanges: true);

            ScriptFileWatcher.WatchForChanges();
            CssFileWatcher.WatchForChanges();
        }

        private static void RunStartupRegistrars<TAttribute>() where TAttribute : BaseRegistrarAttribute
        {
