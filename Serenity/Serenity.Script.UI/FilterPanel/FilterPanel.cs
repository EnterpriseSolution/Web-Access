...
namespace Serenity
{
    public partial class FilterPanel : FilterWidgetBase<object>
    {
        ...
        {
            this.ById("AddButton").Text("add criteria").Click(AddButtonClick);
            this.ById("SearchButton").Text("apply filter").Click(SearchButtonClick);
            this.ById("ResetButton").Text("reset").Click(ResetButtonClick);
        }
            ...
            for (int i = 0; i < rowsDiv.Children().Length; i++)
            {
                try
                {
                    ...
                    if (op == null || op.Length == 0)
                        throw new ArgumentOutOfRangeException("operator", "invalid operator");
        ...
        private jQueryObject AddEmptyRow(bool popupField)
        {
            ...
            jQueryObject andor = parenDiv.Children("a.andor").Attribute("title", "click to switch between AND/OR");
            ...
            row.Children("a.delete").Attribute("title", "remove criteria").Click(DeleteRowClick);
