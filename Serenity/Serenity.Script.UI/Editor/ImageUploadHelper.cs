...
namespace Serenity
{
    ...
    public static class UploadHelper
    {
        ...
        public static bool CheckImageConstraints(UploadResponse file, ImageUploadEditorOptions opt)
        {
            ...
            {
                Q.Alert(String.Format("Uploaded file must be at least {0}!", FileSizeDisplay(opt.MinSize)));
                return false;
            }

            if (opt.MaxSize > 0 && file.Size > opt.MaxSize)
            {
                Q.Alert(String.Format("Uploaded file must be smaller than {0}!", FileSizeDisplay(opt.MaxSize)));
                return false;
            }
            ...
            {
                Q.Alert(String.Format("Uploaded image should be at least {0} pixels in width!", opt.MinWidth));
                return false;
            }

            if (opt.MaxWidth > 0 && file.Width > opt.MaxWidth)
            {
                Q.Alert(String.Format("Uploaded image should be under {0} pixels in width!", opt.MaxWidth));
                return false;
            }

            if (opt.MinHeight > 0 && file.Height < opt.MinHeight)
            {
                Q.Alert(String.Format("Uploaded image should be at least {0} pixels in height!", opt.MinHeight));
                return false;
            }

            if (opt.MaxHeight > 0 && file.Height > opt.MaxHeight)
            {
                Q.Alert(String.Format("Uploaded image should be under {0} pixels in height!", opt.MaxHeight));
                return false;
            }
