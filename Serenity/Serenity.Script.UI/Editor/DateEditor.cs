    ...
    public class DateEditor : Widget<object>, IStringValue, IReadOnly
    {
            ...
            : base(input, new object())
        {
            ...
            input.AddValidationRule(this.uniqueName, e =>
            {
                ...
                if (!string.IsNullOrEmpty(MinValue) && String.Compare(value, MinValue) < 0)
                    return String.Format("Please enter a date after '{0}'.",Q.FormatDate(MinValue));

                if (!string.IsNullOrEmpty(MaxValue) && String.Compare(value, MaxValue) >= 0)
                    return String.Format("Please enter a date before '{0}'.",Q.FormatDate(MaxValue));
