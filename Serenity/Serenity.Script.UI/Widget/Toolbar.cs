...
namespace Serenity
{
    public class Toolbar : Widget<ToolbarOptions>
    {
        ...
        private void CreateButton(jQueryObject container, ToolButton b)
        {
            ...
            if (b.Separator == true)
            {
                J("<div class=\"separator\"></div>").AppendTo(container);
            }

            var btn = J("<div class=\"tool-button\">" +
            ...
            if (!string.IsNullOrEmpty(b.Hotkey) && Script.IsValue(Window.Instance.As<dynamic>().Mousetrap))
            {
