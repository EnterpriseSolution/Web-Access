(function() {
	...
	var $Serenity_DateEditor = function(input) {
		...
		$Serenity_VX.addValidationRule(input, this.uniqueName, ss.mkdel(this, function(e1) {
			...
			if (!ss.isNullOrEmptyString(this.get_minValue()) && ss.compareStrings(value, this.get_minValue()) < 0) {
				return ss.formatString("Please enter a date after '{0}'.", Q.formatDate(this.get_minValue(), null));
			}
			if (!ss.isNullOrEmptyString(this.get_maxValue()) && ss.compareStrings(value, this.get_maxValue()) >= 0) {
				return ss.formatString("Please enter a date before '{0}'.", Q.formatDate(this.get_maxValue(), null));
			}
			return null;
		}));
	...
	$Serenity_EditorUtils.getDisplayText = function(editor) {
		...
		if (ss.isInstanceOfType(value, Boolean)) {
			return (!!value ? ss.coalesce('yes', 'True') : ss.coalesce('no', 'False'));
		}
		return value.toString();
	};
	...
	$Serenity_EditorUtils.setRequired = function(widget, isRequired) {
		...
		if (isRequired && !hasSupItem) {
			$('<sup>*</sup>').attr('title', 'this field is required').prependTo(gridField.find('.caption')[0]);
		}
	...
	var $Serenity_FilterDisplayBar = function(div) {
		...
		this.element.find('.reset').attr('title', 'clear filter');
	...
	$Serenity_GridSelectAllButtonHelper.define = function(getGrid, getId, getSelected, setSelected, text, onClick) {
		return {
			title: ss.coalesce(text, 'Select All'),
	...
	$Serenity_GridUtils.addIncludeDeletedToggle = function(toolDiv, view, hint, initial) {
		...
		var $t1 = ss.coalesce(hint, 'display inactive records');
	...
	var $Serenity_LookupEditorBase = function(hidden, opt) {
		...
		if (!this.options.autoComplete && this.options.inplaceAdd && (ss.isNullOrUndefined(this.options.inplaceAddPermission) || Q.Authorization.hasPermission(this.options.inplaceAddPermission))) {
			this.addInplaceCreate('Define New', null);
		}
	};
	...
	$Serenity_PropertyGridOptions.$ctor = function() {
		...
		$this.defaultCategory = 'Properties';
		return $this;
	};
	...
	var $Serenity_QuickSearchInput = function(input, opt) {
		...
		Serenity.Widget.call(this, input, opt);
		input.attr('title', 'enter the text to search for...').attr('placeholder', 'Search...');
		...
		if (ss.isValue(this.options.fields) && this.options.fields.length > 0) {
			var a = $('<a/>').addClass('quick-search-field').attr('title', 'select the field to search on').insertBefore(input);
	...
	$Serenity_UploadHelper.checkImageConstraints = function(file, opt) {
		if (!file.IsImage && !opt.allowNonImage) {
			Q.alert('Uploaded file is not an image!');
			return false;
		}
		if (opt.minSize > 0 && file.Size < opt.minSize) {
			Q.alert(ss.formatString('Uploaded file must be at least {0}!', $Serenity_UploadHelper.fileSizeDisplay(opt.minSize)));
			return false;
		}
		if (opt.maxSize > 0 && file.Size > opt.maxSize) {
			Q.alert(ss.formatString('Uploaded file must be smaller than {0}!', $Serenity_UploadHelper.fileSizeDisplay(opt.maxSize)));
			return false;
		}
		if (!file.IsImage) {
			return true;
		...
		if (opt.minWidth > 0 && file.Width < opt.minWidth) {
			Q.alert(ss.formatString('Uploaded image should be at least {0} pixels in width!', opt.minWidth));
			return false;
		}
		if (opt.maxWidth > 0 && file.Width > opt.maxWidth) {
			Q.alert(ss.formatString('Uploaded image should be under {0} pixels in width!', opt.maxWidth));
			return false;
		}
		if (opt.minHeight > 0 && file.Height < opt.minHeight) {
			Q.alert(ss.formatString('Uploaded image should be at least {0} pixels in height!', opt.minHeight));
			return false;
		}
		if (opt.maxHeight > 0 && file.Height > opt.maxHeight) {
			Q.alert(ss.formatString('Uploaded image should be under {0} pixels in height!', opt.maxHeight));
			return false;
		}
		return true;
	};
	...
	$Serenity_UploadHelper.colorBox = function(link, options) {
		link.colorbox({ current: 'image {current} / {total}', previous: 'prior', next: 'next', close: 'close' });
	};
	...
	ss.initClass($Serenity_Select2Editor, $asm, {
		...
		emptyItemText: function() {
			return ss.coalesce(this.element.attr('placeholder'), '--select--');
		},
		...
		addInplaceCreate: function(addTitle, editTitle) {
			var self = this;
			addTitle = ss.coalesce(addTitle, 'Define New');
			editTitle = ss.coalesce(editTitle, 'Edit');
		...
		getCreateSearchChoice: function(getName) {
			return ss.mkdel(this, function(s) {
				...
				if (!Q.any(this.get_items(), function(x1) {
					...
					return { id: (-2147483648).toString(), text: '*** No results. Click to define a new one ***' };
				}
				if (this.isAutoComplete()) {
					return { id: this.lastCreateTerm, text: this.lastCreateTerm };
				}
				return { id: (-2147483648).toString(), text: '*** Click to define a new one ***' };
			});
		},
	...
	ss.initClass($Serenity_BaseFiltering, $asm, {
		...
		argumentNull: function() {
			return new ss.ArgumentNullException('value', 'value required!');
		},
	...
	ss.initClass($Serenity_BooleanFormatter, $asm, {
		format: function(ctx) {
			...
			if (!!ctx.value) {
				var $t1 = Q.tryGetText(this.get_trueText());
				if (ss.isNullOrUndefined($t1)) {
					$t1 = ss.coalesce(this.get_trueText(), 'Yes');
				}
				return Q.htmlEncode($t1);
			}
			var $t2 = Q.tryGetText(this.get_falseText());
			if (ss.isNullOrUndefined($t2)) {
				$t2 = ss.coalesce(this.get_falseText(), 'No');
			}
	...
	ss.initClass($Serenity_DataGrid, $asm, {
		...
		createPager: function() {
			...
			pagerDiv.slickPager({ view: this.view, rowsPerPage: 20, rowsPerPageOptions: [20, 30, 50, 100, 500, 2500] });
		},
			...
			return {
				...
				handler: function(args) {
					...
					if (active1 && !Q.parseDate(args.widget.element.val())) {
						active1 = false;
						Q.notifyWarning('Please enter a valid date.', '', null);
				...
				displayText: function(w, l) {
					...
					var text2 = l + ' <= ' + v2;
					if (!ss.isNullOrEmptyString(v1) && !ss.isNullOrEmptyString(v2)) {
						return text1 + ' ' + ss.coalesce('and', 'and') + ' ' + text2;
					}
		...
		booleanQuickFilter: function(field, title, yes, no) {
			var $t1 = $Serenity_SelectEditorOptions.$ctor();
			var $t2 = [];
			$t2.push(['1', ss.coalesce(yes, 'yes')]);
			$t2.push(['0', ss.coalesce(no, 'no')]);
			$t1.items = $t2;
			return {
				field: field,
					...
					if (flags.quickFilterText === true && $(e).hasClass('quick-filter-active')) {
						...
						if (!ss.isNullOrEmptyString(displayText)) {
							if (!ss.isNullOrEmptyString(settings.quickFilterText)) {
								settings.quickFilterText += ' ' + ss.coalesce('and', 'and') + ' ';
	...
	ss.initClass($Serenity_EntityDialog, $asm, {
		...
		getEntityTitle: function() {
			if (!this.isEditMode()) {
				return ss.formatString('New {0}', this.getEntitySingular());
			}
			else {
				var title = ss.coalesce(this.getEntityNameFieldValue(), '');
				return ss.formatString('Edit {0}{1}', this.getEntitySingular(), (Q.isEmptyOrNull(title) ? '' : (' (' + title + ')')));
			}
		},
		...
		showSaveSuccessMessage: function(response) {
			Q.notifySuccess('Save success', '', null);
		},
		getToolbarButtons: function() {
			...
			list.push({ title: 'Apply', hint: 'Apply Changes', cssClass: 'apply-changes-button', hotkey: 'alt+a', onClick: ss.mkdel(this, function() {
			...
			list.push({
				...
				onClick: function() {
					if (self.isDeleted()) {
						Q.confirm('Undelete record?', function() {
			...
			list.push({ title: 'Localization', cssClass: 'localization-button', onClick: ss.mkdel(this, function() {
				this.$localizationButtonClick();
			}) });
			list.push({ title: 'Clone', cssClass: 'clone-button', onClick: ss.mkdel(this, function() {
				if (!self.isEditMode()) {
					return;
				}
		...
		updateInterface: function() {
			...
			if (ss.isValue(this.localizationButton)) {
				...
				this.localizationButton.find('.button-inner').text((this.isLocalizationMode() ? 'Back to Form' : 'Localization'));
			}
			...
			if (ss.isValue(this.saveAndCloseButton)) {
				...
				this.saveAndCloseButton.find('.button-inner').text((this.isNew() ? 'Save' : 'Update'));
			}
	...
	ss.initClass($Serenity_EntityGrid, $asm, {
		...
		getAddButtonCaption: function() {
			return ss.formatString('New {0}', this.getItemName());
		},
		getButtons: function() {
			var self = this;
			var buttons = [];
		...
		newRefreshButton: function(noText) {
			var self = this;
			return {
				title: (noText ? null : 'Refresh'),
				hint: (noText ? 'Refresh' : null),
	...
	ss.initClass($Serenity_EnumEditor, $asm, {
		updateItems: function() {
			...
			for (var $t2 = 0; $t2 < $t1.length; $t2++) {
				var x = $t1[$t2];
				var name = ss.Enum.toString(enumType, x);
				//AddItem(, Q.TryGetText("Enums." + enumKey + "." + name) ?? name);
				//2018/02/13
				var key = String.fromCharCode(ss.unbox(ss.cast(x, ss.Int32)));
				this.addOption(key, ss.coalesce(Q.tryGetText('Enums.' + enumKey + '.' + name), name), null, false);
				//AddItem( Convert.ToChar((int)x).ToString(), Q.TryGetText("Enums." + enumKey + "." + name) ?? name);
				//Enterprise Solution
			}
		},
		getSelect2Options: function() {
	...
	ss.initClass($Serenity_FilterDialog, $asm, {
		...
		getDialogOptions: function() {
			...
			opt.buttons = [{ text: 'OK', click: ss.mkdel(this, function() {
				this.$filterPanel.search();
				if (this.$filterPanel.get_hasErrors()) {
					Q.notifyError('Please fix errors that are marked in red', '', null);
					return;
				}
				this.dialogClose();
			}) }, { text: 'Cancel', click: ss.mkdel(this, this.dialogClose) }];
	...
	ss.initClass($Serenity_FilterDisplayBar, $asm, {
		filterStoreChanged: function() {
			...
			this.element.find('.txt').text('[' + ss.coalesce(displayText, 'none') + ']');
		},
		getTemplate: function() {
	...
	ss.initClass($Serenity_FilterPanel, $asm, {
		...
		$initButtons: function() {
			this.byId('AddButton').text('add criteria').click(ss.mkdel(this, this.$addButtonClick));
			this.byId('SearchButton').text('apply filter').click(ss.mkdel(this, this.$searchButtonClick));
			this.byId('ResetButton').text('reset').click(ss.mkdel(this, this.$resetButtonClick));
		},
			...
			for (var i = 0; i < this.$rowsDiv.children().length; i++) {
				try {
					...
					if (ss.isNullOrUndefined(op) || op.length === 0) {
						throw new ss.ArgumentOutOfRangeException('operator', 'invalid operator');
					}
					var line = {};
		...
		$addEmptyRow: function(popupField) {
			...
			var andor = parenDiv.children('a.andor').attr('title', 'click to switch between AND/OR');
			if (isLastRowOr) {
				andor.addClass('or').text('or');
			}
			else {
				andor.text('and');
			}
			andor.click(ss.mkdel(this, this.$andOrClick));
			row.children('a.delete').attr('title', 'remove criteria').click(ss.mkdel(this, this.$deleteRowClick));
	...
	ss.initClass($Serenity_PropertyDialog, $asm, {
		...
		getDialogButtons: function() {
			var $t1 = [];
			$t1.push({ text: 'OK', click: ss.mkdel(this, this.okClick) });
			$t1.push({ text: 'Cancel', click: ss.mkdel(this, this.cancelClick) });
			return $t1;
		},
		destroy: function() {
			if (ss.isValue(this.propertyGrid)) {
	...
	ss.initClass($Serenity_PropertyGrid, $asm, {
		...
		$createField: function(container, item) {
			...
			if (item.required === true) {
				$('<sup>*</sup>').attr('title', 'this field is required').prependTo(label);
			}
		    ...
		    //enterprise solution
			var lookupattri = this.$determineText(item.lookup, function (prefix3) {
			    return prefix3 + item.lookup;
			});
			if (!Q.isEmptyOrNull(lookupattri)) {
			    element.addClass('inputLookup');
			    element.attr('lookup', lookupattri);
			}
			var filterattri = this.$determineText(item.filter, function (prefix4) {
			    return prefix4 + item.filter;
			});
			if (!Q.isEmptyOrNull(filterattri)) {
			    element.attr('filter', filterattri);
			}
		    //end 
